...
namespace netroute
{
    ...
    void Network::add_router(Router router)
    {
        ...
        const auto result = routers_.emplace(router.id(), std::move(router));
        if (!result.second)
        {
            throw std::invalid_argument("router already exists: " + result.first->first);
        }
        adjacency_[result.first->first];
    }
    ...
    std::vector<std::string> Network::link_ids() const
    {
        std::vector<std::string> ids;
